...
struct LaneConfig {
	...
	float spawn_interval = rand_interval(0.5f, 2.0f);
	...
	void PosInit(Sprites& sprites, Textures& textures) {
		...
		while (file >> LaneConfigOBJ.lane >> LaneConfigOBJ.starting_x >> LaneConfigOBJ.ending_x >> 
			...
			if (LaneConfigOBJ.lane == 1) {
				...
				lanes[1].spawn_interval = rand_interval(0.5f, 2.0f); 
				...
				if (sf::Sprite* sprite = RandomSpawningVehicles(sprites, textures,
					lanes[1].starting_x, lanes[1].position_y, lanes[1].direction)) {
						int int_dir = (LaneConfigOBJ.direction == "left") ? -1 : +1; // convert the string direction to int used for moving
				...
				lanes[2].spawn_interval = rand_interval(0.5f, 2.0f); 
				lanes[2].accumulator = 0.f;

				if (sf::Sprite* sprite = RandomSpawningVehicles(sprites, textures,
					lanes[2].starting_x, lanes[2].position_y, lanes[2].direction)) {
						int int_dir = (LaneConfigOBJ.direction == "left") ? -1 : +1; // convert the string direction to int used for moving
						active.emplace_back(sprite, lanes[2].speed,
						lanes[2].starting_x, lanes[2].ending_x, int_dir);
				}
			}
			else if (LaneConfigOBJ.lane == 3) {
				lanes[3] = LaneConfigOBJ; lanes[3].has_lane = true;
				lanes[3].spawn_interval = rand_interval(0.5f, 2.0f); 
				lanes[3].accumulator = 0.f;

				if (sf::Sprite* sprite = RandomSpawningVehicles(sprites, textures,
					lanes[3].starting_x, lanes[3].position_y, lanes[3].direction)) {
						int int_dir = (LaneConfigOBJ.direction == "left") ? -1 : +1; // convert the string direction to int used for moving
						active.emplace_back(sprite, lanes[3].speed,
						lanes[3].starting_x, lanes[3].ending_x, int_dir);
				}
			}
			else if (LaneConfigOBJ.lane == 4) {
				lanes[4] = LaneConfigOBJ; lanes[4].has_lane = true;
				lanes[4].spawn_interval = rand_interval(0.5f, 2.0f); 
	...
	void Update(float dt, Sprites& sprites, Textures& textures) {
		...
		if (lanes[16].has_lane) {
			...
			if (lanes[16].accumulator >= lanes[16].spawn_interval) {
				...
				lanes[16].spawn_interval = rand_interval(0.5f, 2.0f);
		...
		for (auto& v : active) v.update(dt);
		active.erase(std::remove_if(active.begin(), active.end(),
			[](const ActiveVehicles& v) { return v.ReachedEnd(); }),
			active.end());
	}
private:
	LaneConfig lanes[17]; // 1..16 lanes
	std::vector<ActiveVehicles> active;
};